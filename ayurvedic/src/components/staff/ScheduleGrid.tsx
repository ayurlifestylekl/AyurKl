...
export default function ScheduleGrid({ basePath, detailBase, date, therapists, appts, unassigned, blocks, vaidyas = [], treatments = [], editable = false }: Props) {
  ...
  return (
    <div>
      ...
      <div className="overflow-x-auto overflow-y-visible rounded-xl border border-accent/20 bg-white">
        <div className="flex min-w-full">
          {/* Time gutter */}
          <div className="sticky left-0 z-10 flex-none bg-white" style={{ width: 64 }}>
            <div className="sticky top-16 z-10 border-b border-accent/20 bg-white" style={{ height: HEADER_PX }} />
          ...
          {columns.map((col) => (
            <div key={col.code ?? 'wait'} className="flex-1 border-l border-accent/15" style={{ minWidth: COL_W }}>
              {/* The Unassigned column is the backstop for paid bookings nobody
                  has picked a therapist for yet — it must read as an alert. Sticky
                  so the therapist names stay visible while only the time rows scroll. */}
              <div
                style={{ height: HEADER_PX }}
                className={`sticky top-16 z-10 flex items-center justify-center border-b border-accent/20 px-2 text-center font-heading text-[11px] font-bold uppercase tracking-[0.1em] ${
