...
function ActionTile({ href, icon: Icon, label, detail, external, primary }: ActionTileProps) {
  const className = primary
    ...
    : 'group flex h-full flex-col overflow-hidden rounded-3xl border border-[#6E1023]/8 bg-white p-4 transition-all hover:-translate-y-0.5 hover:border-[#D4AF37]/35 sm:p-5'
    ...
    <a
      ...
      style={{
        boxShadow: primary
          ...
          : '0 1px 0 0 rgba(110,16,35,0.04), 0 12px 30px -18px rgba(110,16,35,0.18)',
      }}
    >
      {primary && (
      ...
      <div className="flex items-start justify-between">
        <span
          className={`flex h-10 w-10 items-center justify-center rounded-2xl ${
            primary ? 'bg-[#D4AF37]/15' : 'bg-[#6E1023]/[0.06]'
          }`}
        >
          <Icon
            className={`h-4 w-4 ${primary ? 'text-[#D4AF37]' : 'text-[#6E1023]'}`}
            strokeWidth={1.8}
          />
        </span>
        {external && (
          <ExternalLink
            className="h-3 w-3 text-[#6E1023]/35 transition-colors group-hover:text-[#D4AF37]"
            strokeWidth={2}
          />
        )}
      </div>
      <div className="mt-4 flex-1">
        ...
        <p
          className="mt-1 font-heading text-[13.5px] font-semibold text-[#6E1023]"
