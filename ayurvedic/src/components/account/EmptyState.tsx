  ...
  return (
    <div className="flex flex-1 flex-col items-center justify-center px-5 py-5 text-center">
      <span className="flex h-9 w-9 items-center justify-center rounded-xl bg-[#163F33]/[0.06]">
        <Icon className="h-4 w-4 text-[#1E5B4B]" strokeWidth={1.6} />
      </span>
      <h3 className="mt-2.5 font-heading text-[13px] font-semibold text-[#163F33]">
        {title}
      </h3>
      <p
      ...
      {secondaryCtaLabel && secondaryCtaHref && (
        <Link
          ...
          className="group mt-2 inline-flex items-center gap-1.5 font-heading text-[11px] font-semibold uppercase tracking-[0.14em] text-[#163F33]/55 underline-offset-4 transition-colors hover:text-[#D4AF37] hover:underline"
        >
          {secondaryCtaLabel}
          <ArrowRight className="h-3 w-3 transition-transform duration-200 group-hover:translate-x-0.5" />
        </Link>
