  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="w-full max-w-md rounded-3xl border border-[#1e3d32]/8 bg-white p-6">
        <h3 className="font-heading text-[16px] font-bold text-[#1e3d32]">Set up two-factor authentication</h3>
        ...
        <div className="mt-4 flex justify-center rounded-2xl border border-[#1e3d32]/10 bg-white p-4">
          {/* qrSvg is a data: URI from Supabase Auth */}
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img src={qrSvg} alt="MFA QR code" className="h-44 w-44" />
        </div>

        <p className="mt-3 break-all rounded-xl bg-[#FAF6EE]/60 px-3 py-2 font-mono text-[11px] text-[#1e3d32]/70">
          Can&apos;t scan? Enter this secret manually: <strong>{secret}</strong>
        </p>

        <label className="mt-4 block font-heading text-[10.5px] font-semibold uppercase tracking-[0.18em] text-[#1e3d32]/55">
          Verification code
        </label>
        <input
          inputMode="numeric"
          ...
          disabled={isPending}
          className="mt-2 w-full rounded-2xl border border-[#1e3d32]/15 bg-white px-4 py-2.5 text-center font-mono text-[20px] tracking-[0.4em] text-[#1e3d32] focus:border-[#D4A373] focus:outline-none focus:ring-2 focus:ring-[#D4A373]/30 disabled:opacity-50"
          placeholder="000000"
          autoFocus
        />
