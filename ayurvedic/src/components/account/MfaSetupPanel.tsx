  ...
  return (
    <div className="rounded-2xl border border-[#1e3d32]/8 bg-white p-4">
      <div className="flex items-start gap-3">
        <span className={`mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-xl ${enrolled ? 'bg-[#2F5D50]/[0.10]' : 'bg-[#1e3d32]/[0.06]'}`}>
          {enrolled ? <ShieldCheck className="h-4 w-4 text-[#2F5D50]" /> : <ShieldOff className="h-4 w-4 text-[#1e3d32]/55" />}
        </span>
        <div className="flex-1">
          <p className="font-heading text-[13px] font-semibold text-[#1e3d32]">
            Two-factor authentication
          </p>
          <p className="mt-0.5 font-body text-[11.5px] text-[#2B2B2B]/65">
            {enrolled
          ...
          <div className="mt-3">
            {enrolled ? (
              ...
              <button
                ...
                disabled={isPending}
                className="rounded-full bg-[#2F5D50] px-4 py-2 font-heading text-[11px] font-bold uppercase tracking-[0.14em] text-white hover:bg-[#264a40] disabled:opacity-50"
              >
