...
export default function CsvImportForm() {
  ...
  return (
    <div className="flex flex-col gap-4">
      <section className="rounded-2xl border border-[#1e3d32]/8 bg-white p-4">
      ...
      <section className="rounded-2xl border border-[#1e3d32]/8 bg-white p-4">
        ...
        />
        <label className="mt-4 block text-[11px] font-semibold uppercase tracking-wider text-[#1e3d32]/70">
          …or paste CSV text
        </label>
        <textarea
          value={csv}
        ...
        <div className="mt-4 flex gap-2">
          <button
            ...
            onClick={submit}
            className="rounded-lg bg-[#2F5D50] px-3 py-1.5 text-[13px] font-semibold text-white hover:bg-[#1e3d32] disabled:opacity-50"
          >
        ...
        {result ? (
          <pre className="mt-3 whitespace-pre-wrap rounded-lg bg-[#FAF6EE]/40 p-3 text-[11.5px] text-[#1e3d32]">
            {result}
          </pre>
        ) : null}
      </section>
