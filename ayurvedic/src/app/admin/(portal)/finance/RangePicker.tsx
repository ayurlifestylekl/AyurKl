...
export default function RangePicker({ start, end }: { start: string; end: string }) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 rounded-2xl border border-[#1e3d32]/10 bg-white p-2.5">
      <input
        type="date"
        value={s}
      ...
      />
      <span className="text-[12px] text-[#2B2B2B]/55">→</span>
      <input
        type="date"
        value={e}
      ...
      <span className="ml-1 flex gap-1">
        <button
          ...
          className="rounded-md border border-[#1e3d32]/15 bg-white px-2 py-1 text-[11px] hover:bg-[#FAF6EE]/60"
        >
          This month
        </button>
        <button
