  ...
  return (
    <div className="mx-auto flex max-w-5xl flex-col gap-4">
      ...
      <section className="rounded-3xl border border-[#1e3d32]/10 bg-white p-5">
            ...
            return (
              ...
              >
                <div className="relative flex h-24 w-full items-end justify-center">
                  <div
                    className="w-3/5 rounded-t bg-[#D4A373]/30"
                    style={{ height: `${accruedH}%` }}
                  />
                  <div
                    className="absolute bottom-0 w-3/5 rounded-t bg-[#D4A373]"
                    style={{ height: `${paidH}%` }}
                  />
                </div>
                ...
                <span className="text-[10.5px] font-semibold text-[#1e3d32]">
        ...
        <div className="mt-2 flex justify-end gap-3 text-[10.5px] text-[#2B2B2B]/65">
          <span className="flex items-center gap-1">
            <span className="h-2 w-3 rounded bg-[#D4A373]" /> Paid
          </span>
          <span className="flex items-center gap-1">
            <span className="h-2 w-3 rounded bg-[#D4A373]/30" /> Accrued
          </span>
        </div>
      </section>
            ...
            <tbody className="divide-y divide-[#1e3d32]/6">
              {commissions.map((c) => (
                <tr key={c.id} className="hover:bg-[#FAF6EE]/30">
                  ...
                  <td className="px-4 py-3 font-mono text-[11.5px] text-[#2B2B2B]/70">
                    {c.orderRef ?? '—'}
                  </td>
...
function EmptyState({ text }: { text: string }) {
  return (
    <div className="rounded-2xl border border-dashed border-[#1e3d32]/15 p-10 text-center font-body text-[12.5px] italic text-[#2B2B2B]/55">
      {text}
    </div>
  )
}
