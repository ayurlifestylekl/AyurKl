...
export function toIcsString(appointment: AppointmentRow, customer: IcsCustomer): string {
  ...
  const lines = [
    ...
    'METHOD:PUBLISH',
    'BEGIN:VEVENT',
    `UID:${appointment.id}@keralaayurvediclifestyle.com.my`,
    ...
    `ORGANIZER;CN=Kerala Ayurvedic Lifestyle:mailto:hello@keralaayurvediclifestyle.com.my`,
    `ATTENDEE;CN=${escapeText(customer.fullName)};RSVP=TRUE:mailto:${customer.email || 'unknown@keralaayurvediclifestyle.com.my'}`,
    'STATUS:CONFIRMED',
    'BEGIN:VALARM',
    'ACTION:DISPLAY',
