export function appointmentConfirmationEmail(input: {
  ...
  <div style="margin:24px 0;padding:18px 20px;border:1px solid rgba(30,61,50,0.1);border-radius:18px;background:#FAF6EE">
    <p style="margin:0;font-weight:700;font-size:16px">${input.treatmentName}</p>
    <p style="margin:6px 0 0;color:#1e3d32">${input.whenLocal}</p>
    <p style="margin:6px 0 0;color:#666">with ${input.doctorName}</p>
  </div>
  <p><a href="${input.href}" style="display:inline-block;background:#2F5D50;color:#fff;padding:12px 22px;border-radius:999px;text-decoration:none;font-weight:700">Manage appointment</a></p>
